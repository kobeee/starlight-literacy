...
const characterId = "yi-fresh-horizon-agent-v3";
const duration = 7.8;
const fps = 24;
...
writeSprite(join(canonicalRoot, "sprites/paper-boat/glide-line"), createBoatFrames(), {
  ...
  notes: "Fresh paper boat action for yi-fresh-horizon-agent-v3; no rabbit or guide-hand continuity from earlier samples.",
});
...
console.log(`Wrote fresh horizon recognition-video candidate to ${buildRoot}`);
...
function createBrief() {
  return {
    ...
    pacingRequirements: {
      ...
      productSurface: "P03 认字页竖屏手机教学舞台；节奏按孩子看清和跟读设计，不按展示视频快切。",
    },
    ...
    finalFrame: {
      description: "晨光水面淡在背景里，大字一、yi、就是一和平平横线居中安静可读。",
...
function createAssetPlan() {
  return {
    ...
    audio: {
      status: "baked",
      script: narrationScript,
      path: `tools/recognition-video/builds/${characterId}/assets/audio/yi-fresh-horizon-narration.mp3`,
    },
    qualityBars: [
      "语义动作先于大字出现，不能一开场直接显示字卡。",
      "纸船和水面是教具，不能抢过最终汉字一。",
      "每个 required sprite 都有 poseContract 和 motionChecks。",
      "核心 cue 间隔不低于 1.4 秒，final hold 不低于 2.3 秒。",
...
function createBuildMeta() {
  return {
    ...
    note: "Fresh non-official test. It deliberately avoids previous rabbit/hand/apple/stick animation ideas and uses a paper-boat/horizon-water memory action.",
  };
}
  ...
  return {
    ...
    evidence: {
      keyframes: [
        ...
        },
        {
          ...
          readability: "pass",
          notes: "final hold 足够，可做 poster/fallback。",
        },
      ],
      memoryMoment: {
        exists: true,
    ...
    assetAssessment: {
      ...
      notes: [
        ...
        "音频临时使用既有 yi soundCue 烘焙轨铺底，不是与本版四句 cue 完全同步的最终旁白。",
      ],
    },
    scores: {
      teachingStructure: 8.2,
      glyphBinding: 8.1,
      glyphAnchor: 8.0,
      pacing: 8.0,
      animationPerformance: 7.1,
      visualQuality: 6.4,
      childAppeal: 6.9,
      audioFit: 6.2,
      technicalCompliance: 8.4,
      officialReadiness: 4.8,
    },
    overallScore: 7.2,
    decision: "rendered-needs-iteration",
    blockers: [
      "程序绘制占位资产不能作为 official 美术。",
      "需要真实暖阳绘本 plate/cutout 和正式语音后才能进入 official-candidate。",
    ],
    nextActions: [
      "用同一 brief 生产真实纸船、水线和晨光水面 cutout。",
      "让产品侧看 review sheet，确认小船水线记忆动作是否比旧路径更有记忆点。",
      "替换正式音频后再跑 --official 产品门禁和 H5 guardrails。",
    ],
  };
}
...
function createNarrationAudio() {
  const target = join(buildRoot, "assets/audio/yi-fresh-horizon-narration.mp3");
  const aiff = join(buildRoot, "assets/audio/yi-fresh-horizon-narration.aiff");
  mkdirSync(dirname(target), { recursive: true });

  const bakedSource = join(repoRoot, "src/clients/mobile-h5/assets/audio/unit-01/yi/soundCue.mp3");
  ...
  const sayResult = spawnSync("say", ["-v", "Tingting", "-o", aiff, narrationScript], { stdio: "ignore" });
  ...
  const tone = spawnSync("ffmpeg", [
    ...
    "lavfi",
    "-i",
    "sine=frequency=440:duration=0.18",
    "-f",
    "lavfi",
    "-i",
    "anullsrc=channel_layout=stereo:sample_rate=48000",
    "-filter_complex",
    `[0:a]volume=0.10,apad=pad_dur=${duration}[a0];[1:a]atrim=0:${duration}[a1];[a0][a1]amix=inputs=2:duration=first`,
    "-t",
    String(duration),
    target,
  ], { stdio: "ignore" });
  ...
  if (tone.status !== 0) {
    console.warn("Warning: failed to create narration audio fallback.");
  }
}
...
<head>
  ...
  <title>一 - Fresh Horizon Agent V3</title>
...
<body>
  <div id="${characterId}" class="stage" data-composition-id="${characterId}" data-start="0" data-width="1080" data-height="1920" data-duration="${duration}">
    ...
    <section id="boat-meaning-scene" class="clip scene boat-scene" data-start="0" data-duration="2.25" data-track-index="2" aria-label="一只小船">
...
${spriteImages(`assets/unit-01/${characterId}/sprites/paper-boat/glide-line`, 10)}
    ...
    <section id="water-binding-scene" class="clip scene water-scene" data-start="2.1" data-duration="1.9" data-track-index="3" aria-label="一条水线">
      <div class="focus-ribbon"></div>
...
${spriteImages(`assets/unit-01/${characterId}/sprites/water/settle-line`, 8)}
    ...
    <section id="flat-line-lock-scene" class="clip scene flat-scene" data-start="3.85" data-duration="1.8" data-track-index="4" aria-label="平平一横">
      <div class="focus-ribbon"></div>
      <div class="horizon-line"></div>
      <div class="cue cue-flat">平平一横</div>
    </section>

    <section id="glyph-close-scene" class="clip scene glyph-scene" data-start="5.45" data-duration="2.35" data-track-index="5" aria-label="就是一">
    ...
    <audio id="narration-audio" class="clip" data-start="0" data-duration="${duration}" data-track-index="20" src="assets/audio/yi-fresh-horizon-narration.mp3" preload="auto"></audio>
  </div>
  ...
  <script>
    ...
    timeline.add((time) => {
      setOpacity(".boat-scene", ease(range(time, 0.0, 0.22)) * (1 - range(time, 2.02, 0.28)));
      setScaleX(".boat-scene .horizon-line", ease(range(time, 0.28, 0.72)));
      showSpriteFrame(".boat-scene .boat", time, 0.2, 1.72, 10);
      setTransform(".boat-scene .boat", {
        x: 470 * ease(range(time, 0.2, 1.72)),
        y: -8 * Math.sin(Math.PI * range(time, 0.2, 1.72)),
        scale: 1,
        rotation: -2 + 3 * Math.sin(Math.PI * range(time, 0.2, 1.72))
      });

      setOpacity(".water-scene", ease(range(time, 2.1, 0.25)) * (1 - range(time, 3.78, 0.25)));
      showSpriteFrame(".water-scene .water", time, 2.28, 1.28, 8);
      setTransform(".water-scene .water", {
        y: -5 * Math.sin(Math.PI * range(time, 2.28, 1.28)),
        scale: 1.02,
        rotation: 0
      });

      setOpacity(".flat-scene", ease(range(time, 3.85, 0.24)) * (1 - range(time, 5.42, 0.25)));
      setScaleX(".flat-scene .horizon-line", ease(range(time, 4.02, 0.62)));

      setOpacity(".glyph-scene", ease(range(time, 5.45, 0.42)));
      setScaleX(".final-line", ease(range(time, 5.75, 0.66)));
      setTransform(".glyph-card", {
        y: 18 * (1 - ease(range(time, 5.45, 0.55))),
        scale: 0.94 + 0.06 * ease(range(time, 5.45, 0.55)),
        rotation: 0
      });
      setOpacity(".final-phrase", ease(range(time, 6.35, 0.35)));
      setOpacity(".tokens", ease(range(time, 6.96, 0.35)));
    });
