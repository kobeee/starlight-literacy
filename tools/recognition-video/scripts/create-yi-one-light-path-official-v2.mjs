...
const characterId = "yi-one-light-path-official-v2";
const duration = 7.4;
const fps = 24;
...
writeSprite(join(canonicalRoot, "sprites/sun/rise-glow"), createSunFrames(), {
  ...
  notes: "One-sun rise teaching overlay for yi-one-light-path-official-v2.",
});
...
function createBuildMeta() {
  return {
    id: characterId,
    name: "一 - One Light Path Official V2",
    characterId,
    char: "一",
    status: "official",
...
function createProductReview() {
  return {
    ...
    sourceArtifacts: {
      ...
      video: `tools/recognition-video/builds/${characterId}/renders/${characterId}.mp4`,
      webm: `tools/recognition-video/builds/${characterId}/renders/${characterId}.webm`,
      reviewSheet: `tools/recognition-video/builds/${characterId}/renders/final-frames/${characterId}-review-sheet.png`,
      poster: `tools/recognition-video/builds/${characterId}/renders/final-frames/${characterId}-poster.png`,
      finalFrame: `tools/recognition-video/builds/${characterId}/renders/final-frames/${characterId}-final.png`,
    },
    evidence: {
      keyframes: [
...
function runProductGate() {
  runNodeScript("scripts/validate-product-video-gate.mjs", ["--official", productReviewPath]);
}

function runHyperFramesQa() {
  const rendersDir = join(buildRoot, "renders");
  const framesDir = join(rendersDir, "final-frames");
  const mp4Path = join(rendersDir, `${characterId}.mp4`);
  const webmPath = join(rendersDir, `${characterId}.webm`);
  const posterPath = join(framesDir, `${characterId}-poster.png`);
  const finalFramePath = join(framesDir, `${characterId}-final.png`);
  const reviewSheetPath = join(framesDir, `${characterId}-review-sheet.png`);

  mkdirSync(rendersDir, { recursive: true });
  mkdirSync(framesDir, { recursive: true });

  runCommand("npx", ["hyperframes", "lint"], { cwd: buildRoot });
  runCommand("npx", ["hyperframes", "inspect", "--at", "0.3,1.6,3.2,4.8,5.8,7.2", "--json"], { cwd: buildRoot });
  runCommand("npx", [
    "hyperframes",
    "render",
    "-o",
  ...
  extractFrame(mp4Path, "0.3", posterPath);
  extractFrame(mp4Path, "7.2", finalFramePath);
  createReviewSheet(mp4Path, reviewSheetPath);
  verifyMedia(mp4Path, { expectAudio: true });
  verifyMedia(webmPath, { expectAudio: true });
}
...
function createReviewSheet(videoPath, outputPath) {
  const frameSpecs = [
    ["0.55", "01"],
    ["2.25", "02"],
    ["4.35", "03"],
  ...
  const sheetFrames = frameSpecs.map(([time, label]) => join(dirname(outputPath), `${characterId}-review-${label}.png`));

  frameSpecs.forEach(([time], index) => extractFrame(videoPath, time, sheetFrames[index]));
  runCommand("ffmpeg", [
    "-hide_banner",
    "-loglevel",
    "error",
    ...
    sheetFrames[3],
    "-filter_complex",
    "[0:v]scale=270:480[a];[1:v]scale=270:480[b];[2:v]scale=270:480[c];[3:v]scale=270:480[d];[a][b][c][d]xstack=inputs=4:layout=0_0|270_0|540_0|810_0[out]",
    "-map",
    "[out]",
    outputPath,
  ], { cwd: repoRoot });
...
function verifyMedia(videoPath, { expectAudio }) {
  ...
  const mediaDuration = Number.parseFloat(probe.format?.duration || video?.duration || "0");

  if (!video || video.width !== 1080 || video.height !== 1920) {
    console.error(`${videoPath}: expected 1080x1920 video stream.`);
    process.exit(1);
  }
  if (!Number.isFinite(mediaDuration) || Math.abs(mediaDuration - duration) > 0.35) {
    console.error(`${videoPath}: duration ${mediaDuration} does not match ${duration}s.`);
    process.exit(1);
  }
  if (expectAudio && !audio) {
    console.error(`${videoPath}: expected an audio stream.`);
    process.exit(1);
  }
}
...
<head>
  ...
  <title>一 - One Light Path Official V2</title>
